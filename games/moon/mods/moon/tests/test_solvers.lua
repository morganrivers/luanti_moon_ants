...
describe("Primitive engine solvers", function()
  ...
  --   ports_registry.add(pos_hash_src, 0, types.POWER, {current_A=0, voltage=5.0}) -- Source: ideal 5V
  --   ports_registry.add(pos_hash_mid, 0, types.POWER, {current_A=0, voltage=0.0}) -- Node B: to measure
  --   ports_registry.add(pos_hash_gnd, 0, types.POWER, {current_A=0, voltage=0.0}) -- GND

  --   -- Add ELECTRIC bonds: SRC <-> MID (R), MID <-> GND (C)
  --   bonds_registry.set(pos_hash_src, 0, pos_hash_mid, 0, {type=bonds_types.ELECTRIC, R=100.0})   -- 100 ohm
  --   bonds_registry.set(pos_hash_mid, 0, pos_hash_gnd, 0, {type=bonds_types.ELECTRIC, C=1e-3})    -- 1 mF
  ...
  it("Mechanical: 2 shafts with 4:1 ratio propagate rpm", function()
    ...
    -- print("pos_hash_B")
    -- print(pos_hash_B)
    -- ports_registry.add(pos_hash_A, 0, class=types.ACTUATOR, state={command=100.0, omega=0})
    ports_registry.add(pos_hash_A, 0, types.ACTUATOR, {command=100.0, omega=0})
    -- print("3")
    ports_registry.add(pos_hash_B, 0, types.ACTUATOR, {command=0.0, omega=0})
    -- print("4")
    -- Add SHAFT bond with ratio
    bonds_registry.add(pos_hash_A, 0, pos_hash_B, 0, {type=bonds_types.SHAFT, ratio=4.0, omega_rpm=0, torque_Nm=0})
    -- Build island
    local island = {
  ...
  --   bonds_registry.add(pos_hash_A, 0, pos_hash_B, 0, {type=bonds_types.THERMAL, k=400}) -- W/mK, copper
  --   bonds_registry.add(pos_hash_B, 0, pos_hash_C, 0, {type=bonds_types.THERMAL, k=400})
  --   -- Build island
  --   local island = {
