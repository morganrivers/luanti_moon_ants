...
local vox_meta    = require("voxels.metadata")
local ports_reg   = require("ports.registry")

local THERMAL_BOND = bonds_types.THERMAL
local POWER_PORT   = require("ports.types").POWER
...
local function sum_joule_heating(voxel, dt)
  ...
  if not port or port.class ~= POWER_PORT then return 0 end
  local I = port.state_tbl.current_A or 0
  local mat = materials.get(voxel.material_id)
  local R = mat and mat.ρ or 0
